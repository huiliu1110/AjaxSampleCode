...
function search()
{
	...
	{
		echo '{"success":false,"msg":"missing keyword"}';
		return;
	}
	...
	$number = $_GET["number"];
	$result = '{"success":false, "msg":"Can not find the employee"}';

	//search in array
	foreach($staff as $value)
	{
		if($value["number"] == $number){
			$result = '{"success":true, "msg":"Find the employee '.$value["name"].'"}';
			break;
		}
	}
	echo $result;	
...
function create(){
	...
	{
		echo '{"success":false,"msg":"Missing Value Error"}';
		return;
	}
	//insert the new employee into array
	echo '{"success":true, "msg":"Create a new employee '.$_POST["name"].'"}';
}
?>